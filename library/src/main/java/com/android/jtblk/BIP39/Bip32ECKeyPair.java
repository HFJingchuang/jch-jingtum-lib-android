...
import com.android.jtblk.BIP44.Index;
import com.android.jtblk.crypto.ecdsa.ECKeyPair;
...
public class Bip32ECKeyPair extends ECKeyPair {
    ...
    public static Bip32ECKeyPair deriveKeyPair(Bip32ECKeyPair master, AddressIndex addressIndex) {
        ...
        int coinType = addressIndex.getParent().getParent().getParent().getValue().coinType();
        int purpose = addressIndex.getParent().getParent().getParent().getParent().getValue();

        curr = curr.deriveChildKey(Index.hard(purpose));
        curr = curr.deriveChildKey(Index.hard(coinType));
        curr = curr.deriveChildKey(Index.hard(account));
        curr = curr.deriveChildKey(change);
        curr = curr.deriveChildKey(address);
    ...
    private Bip32ECKeyPair deriveChildKey(int childNumber) {
        if (!hasPrivateKey()) {
            ...
            ByteBuffer data = ByteBuffer.allocate(37);
            if (Index.isHardened(childNumber)) {
